...
class yolov8_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
       ...
       # method = eval(method)(model, target_layers, use_cuda= =(6'))
...
def get_params():
    params = {
      ...
      #  'device': 'cuda',
        ...
        'backward_type': 'all',  # class, box, all 反向传播的变量这里默认值是all（score+box进行反向传播然后进行梯度求和）
        'conf_threshold': 0.2,  # 0.2 置信度阈值 默认0.6
        'ratio': 0.02,  # 0.02-0.1 取前多少数据 就是只去置信度排序的前百分之二进行计算热力图
        'show_box': True,
        'renormalize': True
    }
    return params
...
if __name__ == '__main__':
    ...
    model('ultralytics/cfg/datasets/CottenWeedsID15/images/val', 'result')
    # model(r'/home/hjj/Desktop/dataset/dataset_visdrone/VisDrone2019-DET-test-dev/images/9999947_00000_d_0000026.jpg', 'result')
   # model('D:\\YoloV8Source\\ultralytics\\cfg\\datasets\\MyDataSets\\images\\test\\10.jpg', 'result')